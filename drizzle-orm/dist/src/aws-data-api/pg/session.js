...
import { PgPreparedQuery, PgSession, PgTransaction, } from '~/pg-core/index.ts';
import { fillPlaceholders, sql } from '~/sql/sql.ts';
import { mapResultRow } from '~/utils.ts';
import { getValueFromDataApi, toValueParam } from '../common/index.ts';
export class AwsDataApiPreparedQuery extends PgPreparedQuery {
    client;
    params;
    typings;
    options;
    ...
    /** @internal */
    transactionId, _isResponseInArrayMode, customResultMapper) {
        ...
        if (!fields && !customResultMapper) {
            ...
            const mappedRows = rows.map((sourceRow) => {
                ...
                for (const [index, value] of sourceRow.entries()) {
                    ...
                    if (!metadata) {
                        throw new Error(`Unexpected state: no column metadata found for index ${index}. Please report this issue on GitHub: https://github.com/drizzle-team/drizzle-orm/issues/new/choose`);
                    }
                    if (!metadata.name) {
                        throw new Error(`Unexpected state: no column name for index ${index} found in the column metadata. Please report this issue on GitHub: https://github.com/drizzle-team/drizzle-orm/issues/new/choose`);
                    }
...
export class AwsDataApiSession extends PgSession {
    ...
    prepareQuery(query, fields, name, isResponseInArrayMode, customResultMapper, transactionId) {
        return new AwsDataApiPreparedQuery(this.client, query.sql, query.params, query.typings ?? [], this.options, fields, transactionId ?? this.transactionId, isResponseInArrayMode, customResultMapper);
    }
    execute(query) {
        return this.prepareQuery(this.dialect.sqlToQuery(query), undefined, undefined, false, undefined, this.transactionId).execute();
    }
...
export class AwsDataApiTransaction extends PgTransaction {
