...
import { getOperators, getOrderByOperators, Many, normalizeRelation, One, } from '~/relations.ts';
...
export class MySqlDialect {
        ...
        if (joins) {
            for (const [index, joinMeta] of joins.entries()) {
                ...
                if (is(table, MySqlTable)) {
                    ...
                    joinsArray.push(sql `${sql.raw(joinMeta.joinType)} join${lateralSql} ${tableSchema ? sql `${sql.identifier(tableSchema)}.` : undefined}${sql.identifier(origTableName)}${useIndexSql}${forceIndexSql}${ignoreIndexSql}${alias && sql ` ${sql.identifier(alias)}`} on ${joinMeta.on}`);
                }
    ...
    buildRelationalQueryWithoutLateralSubqueries({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn, }) {
        ...
        where = and(joinOn, where);
        if (nestedQueryRelation) {
            let field = sql `json_array(${sql.join(selection.map(({ field }) => is(field, MySqlColumn)
